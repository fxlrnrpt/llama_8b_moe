...
7. Examined some papers - better idea! Let's slice FFN into multiple experts along FFN hidden dim (intermediate_size). Then using all of the experts at once should match the output of the original FFN. We can't train them though, but we will have a separate batch of experts for that.
...
15. Testing the final implmentation with untrained router. Replaced router initialization for small std normal ~ nearly uniform routing after softmax. Also increased top_k to 8 to match the initial numer of weigths in dense model. Otherwise, in the routed mode (whne the router and the experts are not trained) the model produces garbage. With the new parameters the output is still far from ideal, but sill better. Also, intuitevely we at least have a chance of something reasonable without a ton of continuous pre-training becasue we match the number of dense FFN params.
16. Time to train the experts! First, ideally, we would train them on the base model first with everything else frozen, then extract the routers and learned experts from the base model, transfer them to the instruct model and apply the post-trainig. This would avoid doing continuous pre-training (potentially, tons of data) on the Instruct model with already altered distribution. Since this is a toy example I'll do the continuous pre-training on the Instrcut version :cry:.
17. Using DP across 2 GPUs to fit in memory. FSDP seems like a natural choice since it is built-in in torch. I think we can go with a larger per-GPU batch size opposed to what Claude currently proposed, but let us try. Also, need to verify the effective batch size from the papers. I have doubts. 
18. Made it finally train on 3 H100 (yes, 3! - had hard time getting 4 on runpod). 2 H100 did provide enough sharding to fit the model with FSDP. Fixed gradient checkpointing by defering the router bias update.